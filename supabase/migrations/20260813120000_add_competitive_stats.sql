create table public.competitive_stats (
  ...
  constraint competitive_stats_rating_floor check (rating >= 100),
  constraint competitive_stats_matches_nonnegative check (matches_played >= 0),
  constraint competitive_stats_rated_matches_nonnegative check (rated_matches >= 0),
  constraint competitive_stats_rated_matches_bounded check (rated_matches <= matches_played),
  constraint competitive_stats_wins_nonnegative check (wins >= 0),
  constraint competitive_stats_losses_nonnegative check (losses >= 0),
  constraint competitive_stats_record_balanced check (matches_played = wins + losses)
);
...
revoke all on table public.competitive_stats from public, anon, authenticated;
grant select on table public.competitive_stats to authenticated;
grant select, insert, update, delete on table public.competitive_stats to service_role;

create policy "Users can read their own competitive stats"
  on public.competitive_stats
  for select
  to authenticated
  using ((select auth.uid()) = profile_id);

create function public.ensure_profile_competitive_stats()
returns trigger
language plpgsql
security definer
set search_path = ''
as $$
begin
  insert into public.competitive_stats (profile_id)
  values (new.id)
...
revoke execute on function public.ensure_profile_competitive_stats()
  from public, anon, authenticated;

create trigger ensure_competitive_stats_after_profile_insert
  after insert on public.profiles
  for each row execute function public.ensure_profile_competitive_stats();
...
create table public.competitive_matches (
  ...
  created_at timestamptz not null default now(),
  constraint competitive_matches_winner check (winner in ('survivors', 'wardens')),
  constraint competitive_matches_player_count check (player_count between 1 and 9),
  constraint competitive_matches_authenticated_player_count
    check (authenticated_player_count between 0 and player_count),
  constraint competitive_matches_time_order check (ended_at >= started_at)
);

create table public.competitive_match_participants (
  ...
  constraint competitive_participant_display_name_length
    check (char_length(display_name) between 1 and 32),
  constraint competitive_participant_role check (role in ('survivor', 'warden')),
  constraint competitive_participant_outcome check (outcome in ('win', 'loss')),
  constraint competitive_participant_rating_before check (rating_before >= 100),
  constraint competitive_participant_rating_after check (rating_after >= 100),
  constraint competitive_participant_rating_delta check (rating_delta between -40 and 40),
  constraint competitive_participant_rating_math
...
revoke all on table public.competitive_matches
  from public, anon, authenticated;
revoke all on table public.competitive_match_participants
  from public, anon, authenticated;
grant select, insert, update, delete on table public.competitive_matches to service_role;
grant select, insert, update, delete on table public.competitive_match_participants to service_role;
