...
### 6.3 Minimap (Fog of War)

A HUD overlay in the bottom-right corner showing a player-centered fog-of-war minimap. Purely client-side — no server changes required.

| Property | Value |
|---|---|
| **View area** | 26×26 tiles rendered to a 28×28 offscreen canvas (2-tile buffer for sub-tile scrolling) |
| **Display scale** | 2× — each tile = 2 screen pixels. Final viewport = 52×52 px |
| **Centering** | Always centered on the local player's current position |
| **Scrolling** | Sub-tile smooth scrolling — fractional pixel offset applied to the sprite each frame via PixiJS mask |
| **Fog of war** | `Uint8Array(mapWidth × mapHeight)`, 0 = hidden, 1 = revealed. Circular reveal radius: **7 tiles** |
| **Canvas redraw** | Only when the player crosses a tile boundary (not every frame). Redraws the full 28×28 buffer. |
| **Player marker** | Gold circle (`0xffcc00`, radius 2) with dark outline, rendered as a PixiJS Graphics fixed at viewport center |
| **UI frame** | Stardew-style wooden border — dark outline (`0x3e2312`), wood body (`0xa36a43`), inner highlight (`0xcd8e5e`), drop shadow |
| **Transparency** | Container alpha = 0.85 to avoid obscuring gameplay |

**Tile colour palette:**

| Tile | Minimap Colour | Hex |
|---|---|---|
| Floor (grass) | Vibrant green | `#6ba63d` |
| Floor shadow | Same as grass (shadows are visual overlays only) | `#6ba63d` |
| Walls / solid | Dark wood-stone | `#594932` |
| Fog (hidden) | Deep foliage tone | `#1d2119` |

**Rendering pipeline:**
1. `revealAround()` — marks tiles within a 7-tile circular radius as visited in the fog array.
2. `redrawCanvas()` — iterates the 28×28 buffer, mapping each pixel to a map tile relative to the player's current tile position. Revealed tiles get their colour; hidden tiles get fog.
3. `ctx.putImageData()` → `texture.source.update()` — pushes the updated canvas to the GPU.
4. Sub-tile offset — the PixiJS Sprite is repositioned each frame using fractional pixel offsets, masked by a `Graphics` rectangle to clip to the 52×52 visible area.

**File:** `packages/client/src/systems/Minimap.ts`
...
│   │   │   ├── systems/
│   │   │   │   └── Minimap.ts        ← Fog-of-war minimap HUD (canvas → PixiJS texture)
│   │   │   └── net/
