...
// at ~20 ticks/sec. Every tick it increments the tick counter and broadcasts
// a TickUpdate to every connected client.
// ─────────────────────────────────────────────────────────────────────────────
...
export class Room {
   ...
   * Sends `RoomJoined` (with initial state) to the joining client.
  ...
  addPlayer(ws: PlayerSocket): void {
    ...
    // Add to game state
    const playerInfo: PlayerInfo = { id: playerId, displayName };
    ...
    const joinMsg: RoomJoinedMessage = {
      ...
      roomId: this.id,
      playerId,
      gameState: { ...this.state, players: [...this.state.players] },
    };
    this.send(ws, joinMsg);

    console.info(
      `[Room:${this.id}] Player joined: ${displayName} (${playerId}) — ${this.playerCount} player(s)`,
    );
   ...
   *   2. (Future) Process queued player inputs.
   *   3. (Future) Run game simulation.
   *   4. Broadcast TickUpdate (current state) to all clients.
   */
  ...
  private tick(): void {
    ...
    // 2. TODO: Dequeue and validate buffered player inputs (Step 3+)
    // 3. TODO: Run game simulation — movement, collision, triggers (Step 3+)

    // 4. Broadcast TickUpdate with current state
