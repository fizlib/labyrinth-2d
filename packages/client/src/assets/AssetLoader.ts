...
/** Prime Pixi's cache without constructing the game scene. */
export function preloadAssets(): Promise<void> {
  assetPreloadPromise ??= Assets.load(GAME_ASSET_PATHS)
    .then(() => undefined)
    .catch((error: unknown) => {
      // Optional authored files already have per-group fallbacks in loadAssets.
      console.warn('[Assets] Runtime preload was incomplete; continuing normally', error);
    });
  ...
  try {
    ...
    forestWallTextures = {
      southFaceRows: await Promise.all(
        FIORWOODS_FACE_ROW_STARTS.map((start) =>
          Promise.all(
      ...
      styleDecorationTextures: Object.fromEntries(
        await Promise.all(
          FIORWOODS_STYLE_DECORATION_IDS.map(
            async (id) => [id, await loadFiorwoodsTile(id)] as const,
          ),
        ),
      ),
    };
