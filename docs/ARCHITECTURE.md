...
- Decorated north-closed T-junctions use the exact seven style-editor rectangles for their bushes, rock, and signpost on both the predicting client and authoritative server.
...
14. From the remaining north-closed E/S/W T-junctions, deterministically select up to 85% for the authored stone-ruin, signpost, and vegetation decoration, always selecting at least one whenever a compatible footprint exists. Each prefab reserves its center, west, east, and south cells and is skipped when any of those cells is occupied by an objective, solid authored obstacle, spawn, hub, portal platform, or another selected prefab; floor-only trap cells may overlap it.
